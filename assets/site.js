...
(async function () {
  ...
  // ✅ NEW: uploads list (generated by GitHub Action)
  ...
  function renderFooter(data, lang) {
    ...
    footer.innerHTML = `
      ...
      data.email || ""
        ...
        ${
          line2Parts.length
            ? `<div class="footer__meta footer__meta--small">${line2Parts.join(
                ' <span class="dot">•</span> '
              )}</div>`
            : ``
        }
        ...
        <section class="section">
          ...
          <a class="igcard" href="${escapeHtml(ig)}" target="_blank" rel="noopener">
            ...
            <div class="igcard__sub">${escapeHtml(
              lang === "ru"
                ? "Открыть профиль и смотреть фото"
                : "Avaa profiili ja katso kuvat"
            )}</div>
          </a>
        </section>
      `;
    ...
    return `
      <section class="section">
        ...
        <div class="section__more">
          <a class="link" href="${escapeHtml(ig)}" target="_blank" rel="noopener">📸 ${escapeHtml(
  ...
  function renderTarjousPage(data, lang) {
    ...
    el.innerHTML = `
      <section class="section">
        ...
        <div class="card card--pad">
          <div class="stack">
            <div><strong>${escapeHtml(ui(lang, "phoneLabel"))}:</strong> <a href="tel:${escapeHtml(
      phoneRaw
    )}">${escapeHtml(data.phone || "")}</a></div>
      ...
      <div class="card card--pad">
        ...
        <div class="stack">
          ${
            iban
              ? `<div class="rowline">
                   <div><strong>${escapeHtml(ui(lang, "ibanLabel"))}:</strong> <span class="mono">${escapeHtml(
                  iban
                )}</span></div>
                   <button class="copybtn" type="button" data-copy="${escapeHtml(
                     iban
                   )}">${escapeHtml(ui(lang, "copyIban"))}</button>
                 </div>
          ...
          ${
            eaddr
              ? `<div><strong>${escapeHtml(ui(lang, "verkkolaskuLabel"))}:</strong> <span class="mono">${escapeHtml(
                  eaddr
                )}</span></div>`
              : ""
          }
          ${
            op
              ? `<div><strong>${escapeHtml(ui(lang, "operaattoriLabel"))}:</strong> ${escapeHtml(op)}</div>`
              : ""
          }
        </div>
      </div>
